...
public class ChatMessagesRepository : IChatMessagesRepository
{
    ...
    private static readonly string _addMessageQuery = """
            INSERT INTO chatMessages (chatRoomId, senderId, receiverId, content, createdAt, isGameInvitation, id)
            VALUES (?, ?, ?, ?, ?, ?, ?);
        """;
    ...
    public async Task<Guid> AddChatMessageAsync(ChatMessage chatMessage)
    {
        ...
        var binded = addMessageStatement.Bind(chatMessage.ChatRoomId, chatMessage.SenderId, chatMessage.ReceiverId, chatMessage.Content, chatMessage.CreatedAt, chatMessage.IsGameInvitation, messageId);
